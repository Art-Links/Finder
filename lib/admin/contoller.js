...
var login = async function (req, res, next) {
    try {
        ...
        var loggedAdmin = await models.User.findOne({
            attributes: {
                exclude: ["createdAt", "updatedAt", "id", "isActive", "deletedAt"]
            },
            where: {
                [Op.and]: [
                    {
        ...
        if (loggedAdmin) {
            if (authService.comparePassword(password, loggedAdmin.password)) {
                console.log("comparePassword", (authService.comparePassword(password, loggedAdmin.password)))
                const token = authService.generateToken(loggedAdmin)
                if (token) {
                    result.success = true
