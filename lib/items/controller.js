...
var store = async function (req, res, next) {
    ...
    if(isCreated) {
        ...
        response.succeess = false
        response.massages.push('you have added the item befor !')
    }
...
var show = async function (req, res, next) {
    ...
    var id = req?.user?.id
    var item = await models.items.findAll({
        where: {
            userId: id
        },
        include: [{
    ...
    if (item) {
        result.data = transformers.ItemTransformer(item)
    } else {
        res.status(404)
